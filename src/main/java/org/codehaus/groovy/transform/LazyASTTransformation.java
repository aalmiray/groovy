...
public class LazyASTTransformation extends AbstractASTTransformation {
    ...
    private static void addHolderClassIdiomBody(BlockStatement body, FieldNode fieldNode, Expression initExpr) {
        ...
        markAsGenerated(declaringClass, declaringClass.addMethod(initializeMethodName, ACC_PRIVATE | ACC_STATIC | ACC_FINAL, fieldType,
    ...
    private static void addMethod(FieldNode fieldNode, BlockStatement body, ClassNode type) {
        ...
        markAsGenerated(declaringClass, declaringClass.addMethod("get" + propName, visibility, type, Parameter.EMPTY_ARRAY, ClassNode.EMPTY_ARRAY, body));
        if (ClassHelper.boolean_TYPE.equals(type)) {
            markAsGenerated(declaringClass, declaringClass.addMethod("is" + propName, visibility, type,
    ...
    private static void createSoftSetter(FieldNode fieldNode, ClassNode type) {
        ...
        if (fieldNode.isStatic()) visibility |= ACC_STATIC;
        ClassNode declaringClass = fieldNode.getDeclaringClass();
        markAsGenerated(declaringClass, declaringClass.addMethod(name, visibility, ClassHelper.VOID_TYPE, params(parameter), ClassNode.EMPTY_ARRAY, body));
    }
